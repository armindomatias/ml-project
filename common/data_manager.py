...
class DataManager:
    ...
    def save_predictions(self, df_pred: pd.DataFrame, current_timestamp: pd.Timestamp) -> None:
        ...
        """
        prediction_path = os.path.join(
            self.config.get("data_manager").get("prod_data_folder"),
            self.config.get("data_manager").get("real_time_prediction_data_name")
        )

        # Determine whether to overwrite or append
        if os.path.exists(prediction_path):
            if current_timestamp == pd.to_datetime(self.config.get("pipeline_runner").get("first_timestamp")):
