...
class TrainingPipeline:
    ...
    def make_target(df: pd.DataFrame, target_params: Dict[str, str]) -> pd.DataFrame:
        """
        Create a shifted target column for forecasting tasks (timeseries model).
        ...
        """
        shift_period = target_params.get("shift_period")
    ...
    def tune_hyperparams(
        ...
        def objective(trial: optuna.Trial) -> float:
            # the search space setup in config 
            ss = self.search_space

            # params to be tested by optuna
            params = {
            ...
            # Train model with params chosen by Optuna in an iteration
            ...
            # See if model its good with RMSE
        ...
        # Train final model on full training data with best parameters (probably better to separate this step in another method)
    ...
    def run(self, df: pd.DataFrame) -> Any:
        ...
        """
        x_train, x_test, y_train, y_test = self.prepare_dataset(df) # make target is inside the prepare_dataset method 
        # not saving the study _ 
