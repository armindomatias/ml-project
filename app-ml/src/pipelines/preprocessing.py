...
class PreprocessingPipeline:
    ...
    @staticmethod
    def rename_columns(df: pd.DataFrame, column_mapping) -> pd.DataFrame:
        """
    ...
    def run(self, df: pd.DataFrame):
        ...
        df = self.rename_columns(df=df, column_mapping=self.config.get("column_mapping"))
        df = self.drop_columns(df=df, columns=self.config.get("drop_columns"))
